...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('✅️Lisa working ' + config.WORKTYPE + ' 𝚗𝚘𝚠 '));
          // thanks to afnanplk
...
*/
                        try {
                            ...
                            if (config.NOLOG == 'off') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ 𝘓𝘐𝘚𝘈 ~______~*' +
                                    '\n\n*👾 ' + error + '*\n\n```Report errors\njoin ⚠️Warning bot not allowed in the group\https://t.me/+uq8Lft49fz40YmU1``` ' 
                                    , MessageType.text);
                            }
                        }
                    }
